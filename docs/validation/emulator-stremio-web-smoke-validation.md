...
- Record the current Git commit SHA or a repo-tracked source hash for the module source.
...
- `window.__STREMIO_TIZENBREW_REMOTE__.getState()` returns an initialized runtime state.
- The diagnostics panel can be opened with `Info` and shows the current path, API availability, key state, Back resolution, exit result, and focus state.
- The injected style marker exists as `style[data-stremio-remote-style='1']`.
- The diagnostics or exit modal DOM markers are present only after the runtime creates module-owned UI.
